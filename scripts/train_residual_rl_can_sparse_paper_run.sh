#!/usr/bin/env bash
# ============================================================================
# train_residual_rl_can_sparse.sh — Residual RL fine-tuning (TD3) for PickPlaceCan (Sparse Reward)
# ============================================================================
#
...
# Uses SPARSE binary rewards (0 every step, 1.0 on successful placement).
#
# Task:   PickPlaceCan (robosuite, single Franka Panda, 7-DOF + gripper)
...
# Horizon: 200 steps  (hardcoded in dexmg.py for "PickPlaceCan" task)
# Reward:  SPARSE binary — 0 every step, 1.0 on successful placement
#          (reward_shaping=false in config)
#          Episode does NOT terminate on success (runs full horizon)
#          With sparse reward, agent must learn to push for task completion
...
#
# Prerequisites:
#   - A trained BC policy uploaded to WandB (from train_bc_can.sh)
#   - Set BASE_WANDB_ID below to "project/run_id" from your BC WandB run
#
# Usage:
#   # Inside container:
#   bash scripts/train_residual_rl_can_sparse.sh
#
#   # From host via launch.sh:
#   ./scripts/launch.sh train_res_rl_can_sparse
#
# Output:
...
# Can horizon=200, so 100K steps ≈ 500 episodes of interaction.
...
#   3 = default
#   5 = recommended for Can with sparse reward (faster propagation)
#   10 = aggressive
N_STEP=5

# Discount factor γ
# With horizon=200 and γ=0.995: γ^200 ≈ 0.368 (reward at horizon still valued)
GAMMA=0.995
...
#   50  = recommended by task-specific config (small but sufficient for Can)
...
# For SPARSE reward with γ=0.995 and horizon=200:
#   max possible return = γ^0 * 1 = 1.0 (success at any step)
#   v_min=0.0, v_max=1.0 covers the full range
V_MIN=0.0
V_MAX=1.0
...
# └─────────────────────────────────────────────────────────────────────────┘
#
# SPARSE REWARD MODE: disable reward shaping, use only binary success signal
...
# ║  17. WANDB                                                             ║
# ╚══════════════════════════════════════════════════════════════════════════╝
WANDB_PROJECT="dexmg-can-residual-rl-sparse-paper"
...
# ============================================================================
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Residual RL Training (SPARSE) — TD3 for PickPlaceCan         "
echo "║  Base policy: ${BASE_WANDB_ID} (${BASE_WT_TYPE})            "
echo "║  Steps: ${TOTAL_TIMESTEPS}  |  UTD: ${UTD}  |  γ: ${GAMMA} "
echo "║  N-step: ${N_STEP}  |  Action scale: ${ACTION_SCALE}       "
echo "║  Reward: SPARSE (−reward_shaping)                            "
...
echo ""
echo "✓ Residual RL training (sparse) complete."
